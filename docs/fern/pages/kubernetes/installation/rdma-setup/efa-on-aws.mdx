...
Without RDMA, disaggregated inference falls back to TCP with severe performance degradation (~98s TTFT vs ~1s with EFA on Llama-3.1-8B at ISL 8000). See [RDMA Setup](overview.md) for the transport overview.
...
<Steps toc={true}>
<Step title="Install the AWS EFA Kubernetes Device Plugin" id="step-1-install-the-aws-efa-kubernetes-device-plugin">
...
<Step title="Verify Host Kernel Modules" id="step-2-verify-host-kernel-modules">
...
<Step title="Build a Dynamo EFA Image" id="step-3-build-a-dynamo-efa-image">
...
<Step title="Configure NIXL Backend" id="step-4-configure-nixl-backend">
...
| **vLLM**        | `--kv-transfer-config '{"kv_connector":"NixlConnector","kv_role":"kv_producer","kv_connector_extra_config":{"backends":["LIBFABRIC"]}}'` on prefill, with `kv_consumer` on decode | UCX → TCP fallback |
...
<Step title="Pod Resource Requests" id="step-5-pod-resource-requests">
...
- [RDMA Setup](overview.md) — transport requirements and platform guides
- [RDMA / InfiniBand on AKS](infiniband-on-azure.mdx) — Azure equivalent
