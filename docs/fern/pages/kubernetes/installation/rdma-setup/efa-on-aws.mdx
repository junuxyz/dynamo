...
| **vLLM**        | `--kv-transfer-config '{"kv_connector":"NixlConnector","kv_role":"kv_producer","kv_connector_extra_config":{"backends":["LIBFABRIC"]}}'` on prefill; use `kv_consumer` on decode | UCX → TCP fallback |
